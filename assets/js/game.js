import {
  ...
  async function displayFlagList() {
    const {
      data,
      error
    } = await supa.from('quiz_data').select('*');
 ...
 data.forEach(arrayFlag => {
   ...
   flagElement.innerHTML = `<div><img class="name-it-img-flag" src="${arrayFlag.flag_img}" alt=""></div> <p class="list-item-country">${arrayFlag.answer[0]}</p>`;
...
function trackIndex() {
  ...
  async function checkAnswer() {
    const {
      data,
      error
    } = await supa.from('quiz_data').select('answer');

    getCurrentAnswer = document.querySelector(".focused-flag p").innerHTML
 

    data.forEach(arrayFlag => {
      currentAnswer = arrayFlag.answer[0]

      if (getCurrentAnswer === currentAnswer) {
        matchingArray = arrayFlag.answer;
  ...
  function displayNextFlag() {
    ...
    if (currentIndex > 192) {
      // Handle the case where currentIndex is negative
      currentIndex = 0;
    }
  ...
  document.getElementById('btn-previous').addEventListener('click', displayPreviousFlag)
  ...
  function startStopwatch(updateFunction, maxDuration) {
    ...
    function displayEndScreen() {
      ...
      let addFlagClass = document.querySelectorAll('.container-flag:not(.answered-flag)');
      let listItem = document.querySelectorAll(".list-item-country");
    ...
    let elapsedTime = numSeconds; // or convert numSeconds to your desired format
...
document.getElementById('btn-submit-score').addEventListener('click', async function () {
  if (points > 0) {
    await safeScore();
    }
...
async function safeScore() {
  ...
  const { data, error } = await supa
    .from('leaderboard')
    .insert([
      { score: points, elapsed_time: elapsedTime, guest_username: displayName },
    ])
    .select();
...
let topElementHeight = 415; // Adjust this value based on your top element
let scoreBarHeight = 160; // Adjust this value based on your top element
